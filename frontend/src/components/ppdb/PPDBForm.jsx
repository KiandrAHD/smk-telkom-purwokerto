...
const PPDBForm = ({ onSuccess }) => {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-5" noValidate>
      ...
      <button type="submit" disabled={submitting} className="w-full rounded-lg bg-primary px-4 py-3 text-sm font-bold text-white transition-colors hover:bg-primary-800 disabled:cursor-not-allowed disabled:opacity-60">{submitting ? 'Mengirim pendaftaran...' : 'Kirim Pendaftaran'}</button>
    </form>
  );
};
